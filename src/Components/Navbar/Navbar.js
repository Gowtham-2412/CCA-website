...
const Navbar = () => {
  ...
  return (
    <header className={`padding-x mt-[1px] ${open ? 'overflow-visible':'overflow-hidden'} absolute z-10 h-[71px] w-full `}>
        <nav className='flex justify-between items-center max-container transition-all'>
            ...
            <ul className={` link-style1 top-0 max-lg:pl-15  lg:opacity-100 max-lg:visibility-hidden max-lg:border-2 max-lg:drop-shadow-xl ${open ? 'right-[0%] ':'right-[-100%]'} ${open ? 'z-10':'z-[-1]'}`} id='header'>
                {navLinks.map(item => (
                    <li key={item.label}
                        ...
                        >
                        <a href={item.href} className='font-inter font-semibold leading-normal text-lg text-nav-black max-lg:hover:drop-shadow-lg max-lg:hover:font-bold ' style={{textDecoration: 'none'}} id='link' >{item.label}</a>
                        <img 
                            src={ChevronRight} 
                            alt="right" 
                            width={15}
